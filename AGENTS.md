...
## Layout: a persistent HUD, a menu drawer, the board fills the screen at every width
- **One shell, not a separate desktop design.** `index.html`'s `.page-shell` also carries a `game-shell` class, which the CSS uses to apply the same app-shell treatment (persistent HUD, hamburger menu, full-bleed board, no page scrolling) at every width, not just narrow ones -- the owner asked for the desktop UI to be as simple as the phone one rather than a permanent two-column layout. `arena.html` shares `.layout`/`.sidebar`/`.board-panel` with the game but has no `game-shell` class (and no hamburger button in its markup), so it keeps its original always-visible desktop sidebar -- scope any further shell changes to `.game-shell` so the AI Arena doesn't lose its only way to reach that sidebar.
...
- **Everything else lives in the sidebar** (`#sidebar`): Match setup (Player 2, New Match, nav links), Play Online, Roster, How to play/Rules (with a `.sidebar-legend` copy of the board legend -- the board's own legend was dropped from `index.html` entirely, since the menu is now its only home at any width). The sidebar is always an off-canvas drawer on the game page (`position: fixed`, slid off with `transform`, toggled by the `.open` class), opened by the hamburger `#menuBtn` and closed by `#closeMenuBtn`, the `#sidebarBackdrop`, Escape, or (as a convenience) picking a roster card or starting a new match. It can scroll within itself if its content runs long -- that's an intentional, contained exception to the no-scrolling rule below.
- **The board dominates the screen, by design, at every width.** Below 720px the HUD bar is kept to one compact row (no card padding, the turn pill truncates with an ellipsis, the ships/stars stat rows and ship-icon strip are dropped) -- every one of those is height a narrower design would have handed to chrome instead of the board. **Pitfall already hit once:** `.hud-bar .scoreboard`'s flex-basis is a *width* hint when the HUD bar is a row (the normal case at every width); do not add a rule that turns the HUD bar into a `flex-direction: column` on narrow screens without also zeroing that basis, or the same number gets reinterpreted as a *height* and the score cards balloon to that many pixels tall, crushing the board into a sliver. Prefer shrinking the row's contents (as done here) over stacking it.
- **The board frame is not the board's square** -- **pitfall already hit twice:** `#board` is a flex-grow item (`flex: 1 1 auto`) that fills whatever space is left after the header/HUD/status; it is deliberately *not* itself given `aspect-ratio: 1`, because a flex-grow main-axis size and an `align-items: stretch` cross-axis size are each independently "definite," and when both are definite the aspect-ratio loses -- it silently stops being square rather than erroring. That was the actual cause of a reported bug ("clicking below a ship selects it" / "the board is only half the screen"): the frame became taller than wide, the SVG's default `preserveAspectRatio` letterboxed its square content inside that non-square box, and `board.js`'s `toBoard()` -- which maps a tap proportionally across the SVG's own bounding box, correctly, assuming no letterboxing -- read every tap low by the letterboxed margin. The fix: `#board` (the frame) gets `container-type: size` instead of `aspect-ratio`, and `.battlefield` (the actual SVG, the real square) is sized off it with `width/height: min(100cqw, 100cqh)` and centered with `position: absolute; top/left: 50%; transform: translate(-50%, -50%)`. This guarantees a true square regardless of the frame's own shape, at every width, with no JS involved. Don't reintroduce `aspect-ratio` on `#board` itself, and don't assume a `--screenshot` looking square proves it *is* square -- verify with computed `getBoundingClientRect()` width vs. height (see the testing note below).
- **No scrolling on the main screen:** `.page-shell` is a `100dvh` flex column (`html, body { overflow: hidden }`, via `html:has(.game-shell)` so only the game page is affected) and the board absorbs whatever vertical space is left after the fixed-height header/HUD/status, so it always fits without the page scrolling. Below 720px, Fire Doomstar and End Turn additionally move into a bar fixed to the bottom of the screen, with matching bottom padding reserved on `.page-shell`.
- **Testing narrow widths in headless Chrome:** `--window-size` below roughly 500px is not reliable for `--dump-dom`/`--screenshot` in this environment (Chrome silently renders at a ~500px-wide viewport regardless of the requested size, then crops the screenshot to it, which looks like correct layout but is actually a crop of a wider one -- this is exactly how the flex-basis bug above passed an earlier visual check). Load the page in an `<iframe style="width:NNNpx;height:NNNpx">` inside a wrapper page instead -- the iframe gets its own real browsing context sized by its own CSS, independent of the outer headless window -- and measure `iframe.contentWindow.innerWidth`, `getBoundingClientRect()`, and computed styles from elements inside `iframe.contentDocument`. This also lets you dispatch synthetic `pointerdown`/`click` events at an exact on-screen pixel (e.g. the center of a rendered ship, from its own `getBoundingClientRect()`) and check the result in the DOM (e.g. does `.select-ring` end up on that same ship's `<g>`) -- a much more direct way to catch a tap-coordinate bug than eyeballing a screenshot. Don't trust a `--screenshot` at a sub-500px `--window-size`, or a screenshot at all, as confirmation of exact hit-testing.
...
- 2026-09-12, session 8 (owner: fix a mobile tap-offset bug; simplify the desktop UI to match the mobile one):
  - Found and fixed the root cause of two owner-reported symptoms ("the board is only half the screen" from the previous session, and "clicking below a ship selects it" this session): `#board`'s `aspect-ratio: 1` lost a fight with its own `flex: 1 1 auto` growth, silently becoming non-square and letterboxing the SVG inside it, which threw off every tap coordinate by the letterboxed margin. Fixed by making `#board` a `container-type: size` frame and sizing the actual SVG off it with `min(100cqw, 100cqh)`, guaranteeing a true square with no JS. See the "Layout" section above.
  - Simplified `index.html`'s desktop view to the same shell as mobile (persistent HUD, hamburger menu, full-bleed board, no page scrolling) via a new `game-shell` class, scoped so `arena.html`'s always-visible desktop sidebar is untouched. Removed the board's own legend from `index.html` (the sidebar's `.sidebar-legend` copy is now its only home, at any width).
  - Verified with the iframe test harness at six sizes (360x780 through 1920x1080): the board's SVG is exactly square at every size (was letterboxed before the fix), and a synthetic tap dispatched at a rendered ship's exact on-screen center now selects that ship at every size (was the reported bug). Confirmed `arena.html`'s desktop layout (2-column, no hamburger) is unchanged. `python tools/run_tests.py` unaffected (33/33; this was a CSS/markup-only change, `board.js`/`game.js` untouched).

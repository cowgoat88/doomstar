...
Published: `https://cowgoat88.github.io/doomstar/` (branch `doomain`). The owner asked to match the mobile GUI to similar tactics games -- better button placement, features in menus instead of scrolling -- then, after seeing it on a real device, reported the board was only about half the screen and should be full-screen, and then (session 8) reported a tap-offset bug on mobile (tapping below a ship selects it) and asked for the desktop UI to be simplified to match the mobile one.
...
- **Everything else moved into a menu.** Match setup, Play Online, Roster and How to play/Rules are in the sidebar, an off-canvas drawer (hamburger button; closed by its own button, the backdrop, Escape, or picking a roster card/starting a new match) at **every width now**, on desktop too (see session 8 below) -- not just below 1024px.
- **The board dominates the screen, at every width.** Session 7 fixed a bug where the board only got a fifth of a phone's screen (a scoreboard width hint became a height demand once the HUD bar stacked into a column). Session 8 found and fixed a second, related bug: the board's own frame was supposed to stay square via `aspect-ratio: 1`, but that quietly lost to the frame's own flex-grow sizing and stopped being square, letterboxing the SVG inside it -- which was also the cause of the reported "tapping below a ship selects it" bug, since every tap coordinate was computed as a fraction of the (wrong, non-square) frame. Fixed by sizing the actual SVG off the frame with CSS container query units (`min(100cqw, 100cqh)`) instead of relying on `aspect-ratio` at all -- see `AGENTS.md`'s Layout section for the full mechanism and the pitfall write-up.
- **Desktop simplified to match mobile (session 8):** the same app shell (persistent HUD, hamburger menu, full-bleed non-scrolling board) now applies at every width via a `game-shell` class on `index.html`, not just below 1024px. `arena.html` (AI Arena) shares the same base CSS classes but not the `game-shell` class, so its desktop two-column view is untouched.
- **Lesson learned, twice now:** a headless-Chrome screenshot below `--window-size` ~500px is unreliable (Chrome renders at a ~500px floor internally and crops the output). The session-8 fix instead used an `<iframe style="width:NNNpx;height:NNNpx">` test harness, which also let synthetic taps be dispatched at a rendered ship's exact on-screen pixel and checked against the DOM -- catching the exact tap-offset bug the owner reported, not just eyeballing a screenshot. See `AGENTS.md`.
- **Verified**: `python tools/run_tests.py` 33/33 (unaffected; this was a CSS/markup-only change). An iframe-harness check at six sizes (360x780 to 1920x1080): the board's SVG is exactly square at every size, a synthetic tap at a ship's exact center selects that ship at every size, and `arena.html`'s desktop layout is unchanged.

### Needs the owner
- **Look at it on a real phone again** to confirm taps now land on the ship you tap, not below it, and take a look at the new desktop layout (hamburger menu instead of a permanent sidebar column).
...
## Done
- Session 8: fixed a mobile tap-offset bug and simplified the desktop UI to match mobile (same persistent-HUD/hamburger-menu/full-bleed-board shell at every width). Root cause was a CSS square-sizing bug shared with the session-7 board-size bug; see "Current focus" above and `AGENTS.md`.
